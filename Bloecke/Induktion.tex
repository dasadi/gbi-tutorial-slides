...
% removed because part of exercise sheet
...
\begin{frame}{Und jetzt ihr}
	...
	\begin{block}{Induktionsvoraussetzung}
		Für ein beliebiges aber festes $n \in \N_+$ gelte: $\sum_{k=0}^{n}{\frac{1}{2^k}} = 2 \* \tuple{1 - \frac{1}{2^{n+1}}}$.
	\end{block}
\end{frame}
...
\begin{frame}{Und jetzt mit Wörtern}
	\begin{block}{Behauptung}
		Seien $A, B$ zwei beliebige Alphabete. Definiere die Funktion $f \from A^* \functionto B^*$,
		\begin{align*}
			f(\eps) &:= \eps \\
